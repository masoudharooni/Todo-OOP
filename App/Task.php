...
class Task extends Model
{
    ...
    public function display(int $folder_id = null, int $id = null)
    {
        ...
        $sql = "SELECT * FROM {$this->table} WHERE user_id = :user_id {$folder_query_condition} {$task_query_condition}";
        $stmt = $this->conn->prepare($sql);
    ...
    public function search(string $char)
    {
        $sql = "SELECT * FROM {$this->table} WHERE title LIKE '%$char%' AND user_id = {$this->user_id}";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
